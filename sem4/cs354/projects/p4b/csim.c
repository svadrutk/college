...
//Global timestamps to track cache statistics in access_data().
int hit_cnt = 0;
int miss_cnt = 0;
int evict_cnt = 0;
...
void init_cache() {      
    ...
    // Initialize each line in each set
    for (int i = 0; i < S; i++) {
        for (int j = 0; j < E; j++) {
            cache[i][j].valid = 0;
            cache[i][j].tag = 0;
...
void access_data(mem_addr_t addr) {  
    double max = 0;		//max timestamp in the set
    double min = INT_MAX; 	//min timestamp in the set

    // Get address of first t bits and the addr tag
	mem_addr_t tBit = addr >> (b + s);
	mem_addr_t tag = tBit & ((int)pow(2, 64 - (b+s)) -1);

	// Get the addr set number and address of the first s bit
	mem_addr_t sBit = addr >> b;
	mem_addr_t set = sBit & ((int)pow(2,s) - 1);

	// Search set for largest timestamp value
	for (int i = 0; i < E; i++) {
		if ( cache[set][i].valid == 1 && cache[set][i].timestamp > max) {
			max = cache[set][i].timestamp;
		}
	}
	
	// If hit, update most recently used line and hit_cnt
	for (int i = 0; i < E; i++) {
		if ( cache[set][i].valid == 1 && cache[set][i].tag == tag) {
			cache[set][i].timestamp = max + 1;
			hit_cnt++;
			return;
		}
	}

	// If miss, set the line and update miss_cnt
	for (int i = 0; i < E; i++) {
	...
	// If there is eviction necessary, find LRU then replace it
	for (int i = 0; i < E; i++) { // find the LRU line in the set
		if ( cache[set][i].valid == 1 && cache[set][i].timestamp < min) {
			min = cache[set][i].timestamp;
		}
	}
	for (int i = 0; i < E; i++) { // evict
		if ( cache[set][i].timestamp == min) {
			cache[set][i].tag = tag;
			cache[set][i].valid = 1;
			cache[set][i].timestamp = max + 1;
			miss_cnt++;
