...
function Car(context, x, y, sz, path) {
    this.size = sz || 0.5; 
    this.frontFaster = 2; 
    this.path = path; 
    this.speed = 0.2; 
...
Car.prototype.drawWheel = function() {
    this.context.beginPath();
    this.context.arc(0, 0, 40, 0, 3 * Math.PI);
    this.context.fill();
    this.context.stroke();
}
...
Car.prototype.draw = function() {
    ...
    if (this.path) {
        ...
        else if(p[0] == 600 & p[1] < 900) {
            dx = 0; 
            dy = 1; 
        }
        else if(p[1] == 900 & p[0] > 200) {
            dx = -1; 
            dy = 0; 
        }
        else if(p[0] == 200 & p[1] > 200) {
            dx = 0; 
            dy = -1; 
        }
        else {
        ...
        // console.log(dx, dy);
        // this.context.transform(dx, dy, 0, 0, 0, 0);
    }
    ...
    this.drawBody();
    // this.context.save();
    this.context.translate(200, 300);
    this.drawWheel();
    ...
    this.drawWheel();
    this.context.rotate(slider.value);
...
Car.prototype.update = function() {
    this.backWheelAngle += 0.15;
    this.frontWheelAngle += 0.15 * this.frontFaster;
}
