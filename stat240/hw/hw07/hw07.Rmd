...
What does each row represent?    The length of the list describing each country. 
What does the value of `n` in row 10 of the tibble represent? 
The 10th row of the tibble, Armenia, has 23 elements describing what it is. 

#### B

Are there any countries with different numbers of elements in their list? Briefly justify your response.

There are no countries with a different number of elements in their list, since
both the minimum and maximum number of elements are the same. 
...
- `map()`: Returns a list 
- `map_chr()`: Returns a list of characters
- `map_dbl()`: Returns a list of doubles
- `map_int()`: Returns a list of integers
- `map_lgl()`: Returns a list with logical elements
