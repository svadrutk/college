...
ZIPS2KILL <- dplyr::pull(ZIPS2KILL, zip)
obesity<- obesity[!obesity$zip == ZIPS2KILL, ]
FINALQ4 <- obesity %>% group_by(zip, sex) %>% summarise(adult_n = sum(pop), obese_n = sum(obese), obese_p = obese_n/adult_n) 
FINALQ4
```
...
education <- education %>% pivot_longer(c(Male, Female), names_to = "sex", values_to = "pct_bach")
```
...
```{r}
education <- education[!education$zip == ZIPS2KILL, ]
