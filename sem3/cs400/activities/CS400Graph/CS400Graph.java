...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.distance = copyPath.distance + extendBy.weight.doubleValue(); // add weight of edge to the distance
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if (vertices.get(start) == null || vertices.get(end) == null) {
            throw new NoSuchElementException("One of the vertices listed does not exist.");
        }
        // add the start path
        Path startPath = new Path(vertices.get(start));
        // make a new linked list of minimum paths to store when popping from the PQ
        LinkedList<Path> minPaths = new LinkedList<Path>();
        // add the start path to minimum paths
        minPaths.add(startPath);
        // linked list of vertices to track visited vertices
        LinkedList<Vertex> visited = new LinkedList<Vertex>();
        // begin iterating through the graph
        queue.add(startPath);
        while (visited.size() != vertices.size()) {
            // if (queue.isEmpty()) { // if queue is empty then throw exception
            // throw new NoSuchElementException("No elements left in the priority queue.");
            // }
            if (!visited.contains(queue.peek().end)) {
                visited.add(queue.peek().end);
                minPaths.add(queue.peek());
            }
            Path path = queue.poll();
            Vertex vert = path.end;
            for (int i = 0; i < vert.edgesLeaving.size(); i++) {
                if (!visited.contains(vert.edgesLeaving.get(i).target)) {
                    Path newPath = new Path(path, vert.edgesLeaving.get(i));
                    queue.add(newPath);
                }
            }
        }
        ...
        for (int i = 0; i < minPaths.size(); i++) {
            if (minPaths.get(i).end == vertices.get(end) && minPaths.get(i).start == vertices.get(start)) {
                return minPaths.get(i);
            }
        }
        for (int i = 0; i < minPaths.size(); i++) {
            System.out.println(minPaths.get(i).dataSequence.toString());
        }
        throw new NoSuchElementException("No minimum paths found between the minimum nodes. ");
    }
